...
SymbolAndOffset SymbolAndOffset::findRealSymbol() const {
  ...
  if (Defined *definedSym = dyn_cast<Defined>(s)) {
    if (auto *isec = dyn_cast<InputSectionBase>(definedSym->getSection())) {
...
SymbolAndOffset SymbolAndOffset::findSymbolForCapabilityRelocation() const {
  ...
  if (auto *isec = dyn_cast_or_null<InputSectionBase>(self->getSection())) {
    if (!isec->file)
      return *this;
    for (Symbol *b : isec->file->getSymbols()) {
      if (auto *d = dyn_cast<Defined>(b)) {
...
static uint64_t getTargetSize(const CheriCapRelocLocation &location,
  ...
  const Compartment &c = location.section->getCompartment();
  ...
  if (targetSize == 0 && !targetSym->isPreemptible) {
    ...
    bool isAbsoluteSym = targetSym->getOutputSection(c) == nullptr;
    ...
    if (targetSym->isUndefWeak() && targetSym->getVA(c, 0) == 0)
    ...
    if (Defined *definedSym =
      ...
      if (definedSym->isSection() &&
          definedSym->getSection()->name.startswith(".rodata.str")) {
    ...
    if (OutputSection *os = targetSym->getOutputSection(c)) {
      ...
      if ((int64_t)def->value < 0 || def->value > os->size) {
        ...
        warn("Symbol " + target.verboseToString() +
             ...
             " but the value (0x" + utohexstr(targetSym->getVA(c)) +
      ...
      uint64_t targetVA = targetSym->getVA(c, offset);
...
static CapRelocType getTargetType(const SymbolAndOffset &target) {
  ...
  if (Symbol *s = dyn_cast<Symbol *>(target.symOrSec)) {
    ...
    isTls = s->isTls();
    os = s->getOutputSection(*defaultCompart);
  } else {
...
void CheriCapRelocsSection::writeToImpl(uint8_t *buf) {
  ...
  for (const auto &i : relocsMap) {
    ...
    if (Symbol *s = dyn_cast<Symbol *>(realTarget.symOrSec))
      targetVA = s->getVA(location.section->getCompartment(), 0);
    else {
...
void MipsCheriCapTableMappingSection::writeTo(uint8_t *buf) {
  ...
  for (const SymbolTableEntry &ste : in.symTab->getSymbols()) {
    ...
    entry.funcStart = sym->getVA(*defaultCompart, 0);
